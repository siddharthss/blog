...
from article.forms import CreateArticle
...
def add_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            Article.objects.create(title=title, user_fk=user_object, pub_date=pub_date, update_date=pub_date, content=content)
            messages.success(request, 'New Article created')
            return redirect(index_view)
        else:
            messages.error(request, 'New Article NOT created "Try again" ' )
            return redirect(index_view)
